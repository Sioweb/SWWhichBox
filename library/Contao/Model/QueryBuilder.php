...
class QueryBuilder extends \Contao\Model\QueryBuilder
{
	...
	public static function find(array $arrOptions)
	{
		...
		{
			$strQuery = "SELECT ".($arrOptions['select']?$arrOptions['select']:'*')." FROM " . $arrOptions['table'];
		}
		else
		{
			$arrJoins = array();
			...
			// Generate the query
			$strQuery = "SELECT ".($arrOptions['addSelect']?$arrOptions['addSelect'].',':''). implode(', ', $arrFields) . " FROM " . $arrOptions['table'] . implode("", $arrJoins);
		}
